...
Lista* insereFim(Lista* l, int v){
	...
	p->prox = novo;
	cout << "Adicionando " << v << " ao fim da lista." << endl;
	return l;
}
...
Lista* removeInicio(Lista* l){
	...
	p = l->prox;
	free(l);
	cout << "Removendo primeiro nó da lista." << endl;
	return p;
}
...
// Imprime os valores dos elementos de forma recursiva 
void imprime_recursiva(Lista* l){
	if (vazia(l) == true )
		return;
	// Imprime o elemento da lista atual 
	cout << l->info << ", ";
	// Chama a próxima lista 
	imprime_recursiva(l->prox);
}

// Retira um elemento da lista através de uma checagem recursiva
